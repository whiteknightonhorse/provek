...
// The one collapse of a subject_id into a slug now lives in discovery.mjs (slugOf) - see its own
// header for why a second copy of this regex would be the drift LAW #ONE-PLACE names.
const slug = slugOf;
...
written.push(write("/registry/", page("/registry/", TITLES["/registry/"],
  REGISTRY_SENTENCE,
  ldRegistry())));
// THE MARKDOWN SIBLING (ABI content axis, Fable's ruling 2026-08-31). A rendering of the SAME
// registry.json this HTML page renders, generated by web/markdown.mjs - never hand-written, for
// the reason that file's own header states. web/functions/_middleware.js is what decides which of
// the two a given request receives; this line only makes sure the sibling exists to choose from.
writeFileSync(join(DIST, "registry", "index.md"), buildRegistryMarkdown(registry, SITE));
...
for (const row of registry.subjects) {
  ...
  // Markdown sibling, same reasoning as the registry's above - one passport, two renderings, one
  // input (this passport object), never a hand-maintained third copy.
