...
THE RULING'S TWO HALVES, CHECKED SEPARATELY.
  ...
  2. On the four funnel surfaces this task rewrote - `/`, `/apply/`, `/build/`, `/registry/` -
     ...
     both). `web/src/copy.ts`'s `INCUBATOR_SENTENCE` is what actually satisfies this on all four;
...
scratch copy before the real tree is trusted to have none.
"""
...
def test_the_beside_the_limit_check_catches_an_isolated_mention(tmp_path):
    ...
    reading = _funnel_surface_reading(scratch, "/")
